...
const StudentMarkTable = () => {
  ...
  const [marks, setMarks] = useState<MarksResponse[] | undefined>(undefined);
      ...
      <Box className={styles.buttonBox}>
        {[1, 2].map((sem) => (
          <Button
            ...
            color='primary'
            variant='contained'
            sx={{ borderRadius: '24px', width: '150px', textTransform: 'none', backgroundColor: semester === sem ? '#B6165E' : null }}
          ...
          <TableBody>
            {marks?.map((mark) => (
              <TableRow key={mark.subject.title}>
                ...
                {examHeaders.map((examHeader, columnIndex) => (
                  <StyledTableCell key={columnIndex} align="center">
                    {mark.assignmentMarks[columnIndex]?.mark !== undefined && mark.assignmentMarks[columnIndex]?.mark !== 0
                      ? mark.assignmentMarks[columnIndex].mark
                      : "Not"}
                  </StyledTableCell>
                ))}
                <StyledTableCell>
